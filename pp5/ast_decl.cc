...
Scope *ClassDecl::PrepareScope()
{
    ...
    if (extends) {
        ...
        if (ext) {
          ...
          // copy parent's obj layout
          objLayout.AppendAll(ext->objLayout);
        }
    }
    ...
    // add parent's fields at correct position
    for (int i = 0; i < objLayout.NumElements(); i++) {
        AddField(objLayout.Nth(i));
    }
    for (int i = 0; i < members->NumElements(); i++) {
      Decl* m = members->Nth(i);
      if (m->IsVarDecl())
        objLayout.Append(m);
      AddField(m);
    }
