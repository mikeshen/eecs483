...
SwitchStmt::SwitchStmt(Expr *e, List<CaseStmt*> *c, DefaultStmt *ds) {
    Assert(e != NULL && c != NULL && ds != NULL);
    (eval = e)->SetParent(this);
    (cases = c)->SetParentAll(this);
    (defaultStmt = ds)->SetParent(this);
}

void SwitchStmt::PrintChildren(int indentLevel) {
    eval->Print(indentLevel + 1);
    cases->PrintAll(indentLevel + 1);
    defaultStmt->Print(indentLevel + 1);
}

CaseStmt::CaseStmt(Expr *ic, List<Stmt*> *s) {
